...
**What we shop and what we build are not the same thing.** What we shop is a capability that already works — usually built on a previous customer's contract — demonstrated against a new customer's stated mission requirement. That demonstration is what establishes it applies. The MVP is the work that follows: the *next* customer funding the **tailoring** that adapts the capability into **their mission space**, which is a different organization with different requirements, data, and workflows even when the underlying problem is the same. Both take real time, and both are necessary. A [side-by-side comparison]({{ '/docs/07-glossary-and-references.html' | relative_url }}) is in the glossary.
