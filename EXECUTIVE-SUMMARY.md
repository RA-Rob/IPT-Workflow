...
2. **The `stage` policy — endorsement of it, and its default data class.** Stage lives in Red Alpha's controlled environment (on-prem or cloud), is built to the customer's CONOPS or mirrors our intended production deployment, serves pre-production *and* pre-sales, and may be replicated only at mainline release parity — we need that endorsed as policy. Still undecided: what data class stage may hold by default, and who approves the exception when a customer wants real operational data in it.
