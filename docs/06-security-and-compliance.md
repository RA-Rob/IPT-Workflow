...
**Where the data comes from, and the default data class.** The default is **synthetic**. That default holds outright whenever there is no active funding customer to supply anything else — during a pre-Gate-0 POC, and for as long as a product is still being shopped for its funding customer — because there is no one yet to grant an exception. Once a funding customer is engaged, they may supply their own data for evaluation instead of the synthetic default; where they choose not to, Red Alpha continues to provide synthetic data consistent with the intended production deployment. Either way, the *class* of data in use is recorded in the charter — a field that may not be left blank — because it sets stage's control baseline. Customer-supplied does not automatically mean *operational*: if genuinely operational data lands in stage, stage is inside the authorization boundary from that moment on, and the Security Lead (see *Who owns what*, below) confirms the resulting control baseline and agrees the handling regime in writing *before* the data arrives.
...
**A POC being shopped runs in a stage instance too — there is no separate POC demo environment.** A pre-Gate-0 POC has no product mainline yet, so the release-parity rule has nothing to hold it to; the instance simply reflects the POC as it currently stands, and parity begins to apply the moment a real mainline exists.
