...
|--------------|-------------------------|
| *Before Gate 0 (IP origin & clearance — document 01)* | Not run through this document's RMF machinery — that starts at Gate 0, once a customer is real — but two judgments belong here and we should insist on both. First, security: is a production version of this capability **plausibly authorizable at all**? Shopping something we could never field wastes a customer's time and ours. Second, and non-negotiable, **RA Legal's IP clearance** — capability derived from another customer's contract is not demonstrated to anyone until Red Alpha's right to use it is established in writing (document 01). The demonstration itself stays a demonstration: Red Alpha-controlled environment, synthetic data, nobody operating it. |
...
**Where the data comes from, and the default data class.** The default is **synthetic**. That default holds outright whenever there is no active funding customer to supply anything else — during a pre-Gate-0 demonstration, and for as long as a product is still being shopped for its funding customer — because there is no one yet to grant an exception. Note that the originating customer of any contract-derived IP is *not* a customer who can grant that exception: they are not the funding customer of this engagement, and their data is out of scope regardless (see below). Once a funding customer is engaged, they may supply their own data for evaluation instead of the synthetic default; where they choose not to, Red Alpha continues to provide synthetic data consistent with the intended production deployment. Either way, the *class* of data in use is recorded in the charter — a field that may not be left blank — because it sets stage's control baseline. Customer-supplied does not automatically mean *operational*: if genuinely operational data lands in stage, stage is inside the authorization boundary from that moment on, and the Security Lead (see *Who owns what*, below) confirms the resulting control baseline and agrees the handling regime in writing *before* the data arrives.
...
**It is pre-production and pre-sales — and it is not the pilot.** Two legitimate uses share this environment before a product reaches production: pre-production evaluation by a funding customer's operators (Phase 2 onward), and pre-sales demonstration to a prospective customer while the product is still being built. Both are *evaluation*. Neither carries authorization to operate or any mission dependency, and both sides should be clear that using stage is not using the product for real work — Gate 3 exists precisely because moving to the customer's production environment is a different decision with a different bar. Note that the pre-sales use makes the data question sharper, not looser: a prospect-facing instance must not hold another customer's data.

**Pre-Gate-0 demonstrations run in a stage instance too — there is no separate demo environment for them.** Whether we are showing cleared contract-derived IP or a self-funded POC, it runs in a stage instance. A pre-Gate-0 demonstration has no product mainline yet, so the release-parity rule has nothing to hold it to; the instance simply reflects the capability as it currently stands, and parity begins to apply the moment a real mainline exists.

**The originating customer's data never travels with the IP.** This is the sharpest security consequence of sourcing capability from contract work (document 01), and it is worth stating as a rule rather than leaving to judgment: an IP clearance establishes Red Alpha's right to use a **capability**. It grants nothing at all with respect to the originating customer's data, and it never will — their data, their specific deliverable, and their mission details stay theirs regardless of how the capability itself is cleared. A stage instance demonstrating contract-derived IP therefore runs on **synthetic data**, generated for the purpose, with the originating customer's data excluded as a matter of policy and not of care. The same applies in the other direction, for the same reason: a prospect-facing instance must not hold any other customer's data either.

Practically, this means someone has to look. Capability carried over from a delivered system can arrive with fixtures, seed data, test files, configuration, or log samples still attached to it. Stripping those is part of packaging cleared IP for demonstration, not an afterthought — and it is the Security Lead's read to confirm before the instance is shown to anyone outside Red Alpha.

**Once a product is actually in production, ongoing sales demonstrations move out of stage.** A dedicated **demo environment** — owned by business development, running the current production release — is stood up after the product's first launch, specifically so business development can pitch the shipped product to whichever customer comes next. It is a different thing from stage: stage supports an active engagement's evaluation loop (a funding customer, or a pre-Gate-0 POC being shopped); the demo environment sells a product that already exists.
...
- **Who confirms that contract-derived IP arrived clean** — stripped of the originating customer's data, fixtures, and configuration — before a demonstration instance is shown outside Red Alpha? The rule above puts it on the Security Lead's read; whether that is the right owner, and what the check actually consists of, is not settled.
