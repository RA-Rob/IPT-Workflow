...
**Cleared IP** — IP that **RA Legal has determined Red Alpha has the legal right to use**, on one of the three positions listed under **IP clearance**. It is the asset the front end of the workflow runs on, and the shorthand used throughout these documents for "ours to shop, demonstrate, and build on."

*This term does not mean security-cleared.* In a defense and government context "cleared" usually refers to personnel security clearances, classification levels, or public-release review. **None of that is meant here.** Clearance in this workflow is a determination about *ownership and permission* only. The two dimensions are independent and both may apply to the same capability: legally cleared IP may still be classified, and unclassified IP may still not be ours. Neither determination substitutes for the other, and the people who make them are different. Where these documents mean the security sense, they say so explicitly.
...
**MVP (Minimum Viable Product)** — The smallest version of the **core product**, **tailored** to one named customer's environment and mission, that that customer can put in front of their own operators and judge against their own work. Built with the **funding customer's** money *and their participation*: each short **cycle** delivers working capability into the **stage** environment, the customer evaluates it, and their response shapes the next cycle. Its job is not to establish that the capability applies — that was settled before Gate 0, and is why the customer committed funding — but to work out **what it takes to make it fit a mission space it was not built for**, incrementally, so the adaptation is corrected cheaply and often rather than defended at the end. The MVP is not thrown away: it becomes the customer's delivered instance. See **The shoppable asset vs. the MVP** below for the contrast, and note that the **core product** underneath remains Red Alpha-owned and licensed no matter who funded the tailoring around it.

**Mission space** — The organizational and operational context a capability has to work inside: whose mission it is, what their requirements and authorities are, what data and systems they have, and how their people actually work. Two customers can share a *class* of problem while occupying genuinely different mission spaces — Army Cyber and Navy Cyber, say — which is why a capability proven in one still needs **tailoring** to work in the other. The gap between mission spaces is what the **MVP** phase exists to close; it is not a question of whether the capability applies.
...
**Tailoring (also "fringe" development)** — The customer-funded, customer-specific work that adapts the **core product** to one customer's **mission space**: their integrations, data feeds, deployment constraints, and mission-specific workflows. It is what turns a capability that demonstrably applies to their mission into one that actually works in it — and it is precisely what the customer is paying for, because it is the part only their environment can reveal. Tailoring is not assumed to belong in the core; whether any given piece is **upstreamed** is a separate, recorded decision.
...
|---|---|---|
| **Question it answers** | "Is this real — does the capability work, and does it address a mission requirement someone has?" | "What does it take to make this work in *my* mission space?" — not *whether* it applies, which the demonstration already established |
...
The through-line: the shoppable asset is Red Alpha showing a capability that already works and establishing that it applies here — usually on the strength of work a previous customer already funded, which is what makes the model compound. An MVP is the next customer funding the work of adapting that capability into *their* mission space — a different organization with different requirements, data, and workflows, solving the same class of problem. It is delivered in short, reviewable increments because that gap is easier to close by use than by specification, not because either side is still deciding whether the capability was the right one. Full phase-by-phase detail is in [`05-process-timeline-and-phases.md`](05-process-timeline-and-phases.md); where the IP comes from and what clearing it requires is in [`01-ip-origin-and-clearance.md`](01-ip-origin-and-clearance.md).
