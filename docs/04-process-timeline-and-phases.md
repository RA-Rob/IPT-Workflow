...
5. **Shape the first MVP cycle, and stand up stage.** The Tech Lead sketches the architecture at low fidelity; the Security Lead drafts the control and authorization approach (document 05). The Delivery/Platform hat stands up the `dev → stage → prod` path so that the **stage** environment is ready to receive the first increment — the customer cannot evaluate what they cannot reach.
...
- **Where stage lives:** Red Alpha-hosted or in the customer's environment? This changes the authorization boundary materially, and it changes who can see the data in it. (See document 05.)
